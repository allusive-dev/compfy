...
DESCRIPTION
-----------
compfy is a compositor based on Dana Jansens' version of xcompmgr (which itself was written by Keith Packard). It includes some improvements over the original xcompmgr, like window frame opacity and inactive window transparency.
...
*--config* 'PATH'::
	Look for configuration file at the path. See *CONFIGURATION FILES* section below for where compfy looks for a configuration file by default. Use `/dev/null` to avoid loading configuration file.
...
*--opacity-rule* 'OPACITY':'CONDITION'::
	Specify a list of opacity rules, in the format `PERCENT:PATTERN`, like `50:name *= "Firefox"`. compfy-trans is recommended over this. Note we don't make any guarantee about possible conflicts with other programs that set '_NET_WM_WINDOW_OPACITY' on frame or client windows.
...
*--xrender-sync-fence*::
	Use X Sync fence to sync clients' draw calls, to make sure all draw calls are finished before compfy starts drawing. Needed on nvidia-drivers with GLX backend for some users.
...
This secion describes the interface of a custom shader, how it is used by compfy, and what parameters are passed by compfy to the shader. This does not apply to the legacy backends.

A custom shader is a GLSL fragment shader program, which can be used to override the default way of how a window is rendered. If a custom shader is used, the default compfy effects (e.g. dimming, color inversion, etc.) will no longer be automatically applied. It would be the custom shader's responsibility to apply these effects.

The interface between compfy and a custom shader is dependent on which backend is being used. The xrender backend doesn't support shader at all. Here we descibe the interface provided by the glx backend.

The shader must define a function, 'vec4 window_shader()', which would be the entry point of the shader. The returned 'vec4' will be used to set 'gl_FragColor'. A function, 'vec4 default_post_processing(vec4 c)', is provided for applying the default compfy effects to input color 'c'.
...
The default behavior of compfy window rendering can be replicated by the following shader:
...
CONFIGURATION FILES
-------------------
compfy could read from a configuration file if libconfig support is compiled in. If *--config* is not used, compfy will seek for a configuration file in `$XDG_CONFIG_HOME/compfy.conf` (`~/.config/compfy.conf`, usually), then `$XDG_CONFIG_HOME/compfy/compfy.conf`, then `$XDG_CONFIG_DIRS/compfy.conf` (often `/etc/xdg/compfy.conf`), then `$XDG_CONFIG_DIRS/compfy/compfy.conf`.

compfy uses general libconfig configuration file format. A sample configuration file is available as `compfy.sample.conf` in the source tree. Most of commandline switches can be used as options in configuration file as well. For example, *--vsync* option documented above can be set in the configuration file using `vsync = `. Command line options will always overwrite the settings in the configuration file.
...
* compfy reinitializes itself upon receiving `SIGUSR1`.

D-BUS API
---------

It's possible to control compfy via D-Bus messages, by running compfy with *--dbus* and send messages to `com.github.chjj.compton.<DISPLAY>`. `<DISPLAY>` is the display used by compfy, with all non-alphanumeric characters transformed to underscores. For `DISPLAY=:0.0` you should use `com.github.chjj.compton._0_0`, for example.
...
+
------------
$ compfy --config /dev/null
------------

* Run compfy with client-side shadow and fading:
+
------------
$ compfy -cf
------------
...
+
------------
$ compfy -bcf -i 0.8 -e 0.8 --no-fading-openclose
------------

* Draw white shadows:
+
------------
$ compfy -c --shadow-red 1 --shadow-green 1 --shadow-blue 1
------------

* Avoid drawing shadows on wbar window:
+
------------
$ compfy -c --shadow-exclude 'class_g = "wbar"'
------------

* Enable VSync with GLX backend:
+
------------
$ compfy --backend glx --vsync
------------

BUGS
----
Please submit bug reports to <https://github.com/yshui/compfy>.
...
SEE ALSO
--------
*xcompmgr*(1), link:compfy-trans.html[*compfy-trans*(1)]
